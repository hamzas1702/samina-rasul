...
function sr_order_terms_html() {
	return '<ul class="sr-order-terms">'
		. '<li>' . esc_html__( 'Orders are confirmed with a 50% advance — 100% for international orders.', 'samina' ) . '</li>'
...
add_action( 'woocommerce_before_checkout_form', function () {
	wc_print_notice(
		esc_html__( 'A minimum 50% advance confirms your order (100% for international orders). Made-to-order pieces ship in the delivery window shown on each product.', 'samina' ),
		'notice'
	);
} );
